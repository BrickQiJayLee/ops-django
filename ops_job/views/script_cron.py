...
def get_cron_list(request):
    ...
    cron_list_ret = [{
        "task_name": i['name'],
        "job_name": get_job_name(json.loads(i['kwargs']).get('script_name','无')),
        "script_name": json.loads(i['kwargs']).get('script_name','none'),
...
def commitcron(request):
    try:
        ...
        search_name = task_name if task_name == old_name else old_name
        if search_name is None:
            search_name = task_name
        # 查找任务
...
def get_task_list(request):
    ...
    return HttpResponse(json.dumps({"result": "success", "tks": tks}))


@csrf_exempt
def enablecron(request):
    try:
        job_name = request.POST.get('task_name', None)
